"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var express_1 = require("express");
var client_1 = require("@prisma/client");
var userRouter = (0, express_1.Router)();
var db = new client_1.PrismaClient();
//logging util
var out = function (s) { return console.log(s); };
userRouter.post("/login", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, result, e_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 5, , 6]);
                _a = req.body, email = _a.email, password = _a.password;
                return [4 /*yield*/, db.user.findUnique({
                        where: {
                            email: email
                        }
                    })];
            case 1:
                result = _b.sent();
                if (!((result === null || result === void 0 ? void 0 : result.password) == password)) return [3 /*break*/, 3];
                req.session.user;
                return [4 /*yield*/, req.session.save()];
            case 2:
                _b.sent();
                res.status(200).send({
                    success: true,
                    user: { id: result === null || result === void 0 ? void 0 : result.id, name: result === null || result === void 0 ? void 0 : result.name },
                    msg: "login success"
                });
                return [3 /*break*/, 4];
            case 3:
                res
                    .status(200)
                    .send({ success: false, msg: "invalid login credentials" });
                _b.label = 4;
            case 4: return [3 /*break*/, 6];
            case 5:
                e_1 = _b.sent();
                console.error(e_1 + " in /login");
                res.status(500).send({ success: false, msg: "our bad, server error" });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); });
userRouter.post("/signup", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, username, password, email, result, e_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 4, , 5]);
                _a = req.body, username = _a.username, password = _a.password, email = _a.email;
                return [4 /*yield*/, db.user.findFirst({
                        where: {
                            email: email
                        }
                    })];
            case 1:
                result = _b.sent();
                if (result !== null) {
                    out("duplicate registration");
                    return [2 /*return*/, res.send({
                            success: false,
                            msg: "someone already exists with that username"
                        })];
                }
                return [4 /*yield*/, db.user.create({
                        data: {
                            name: username,
                            email: email,
                            password: password
                        }
                    })];
            case 2:
                result = _b.sent();
                req.session.user = result;
                return [4 /*yield*/, req.session.save()];
            case 3:
                _b.sent();
                res.status(200).send({
                    success: true,
                    user: { id: result === null || result === void 0 ? void 0 : result.id, name: result === null || result === void 0 ? void 0 : result.name },
                    msg: "signup success"
                });
                return [3 /*break*/, 5];
            case 4:
                e_2 = _b.sent();
                console.error(e_2 + " in /signup");
                res.status(500).send({ success: false, msg: "our bad, server error" });
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); });
userRouter.post("/details", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, result, e_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                id = req.body.id;
                out(req.session);
                return [4 /*yield*/, db.user.findFirst({
                        where: {
                            id: id
                        },
                        select: {
                            incomingQueues: {
                                select: {
                                    title: true
                                }
                            },
                            friends: {
                                select: {
                                    name: true
                                }
                            },
                            sentQueues: {
                                select: {
                                    title: true
                                }
                            },
                            profile: {
                                select: {
                                    bio: true
                                }
                            }
                        }
                    })];
            case 1:
                result = _a.sent();
                //want to get a user's...
                //  queues
                //  friends
                //  profile (bio, etc)
                if (result === null) {
                    return [2 /*return*/, res.send({
                            success: false,
                            msg: "nobody exists with that id"
                        })];
                }
                res.send({
                    success: true,
                    details: result
                });
                return [3 /*break*/, 3];
            case 2:
                e_3 = _a.sent();
                console.error(e_3 + " in /signup");
                res.status(500).send({ success: false, msg: "our bad, server error" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
exports["default"] = userRouter;
